...
// Alexis Oakes' on-market listings (Active, Pending, Under Contract, Coming
// Soon), and returns compact JSON with each listing's real status so the site
// labels them correctly. Falls back to a verified snapshot if the feed is
...
// Verified snapshot (source: aoakes.cbtulsa.com). Statuses confirmed by Alexis.
const FALLBACK = [
  { address:"8819 S 74th East Avenue", city:"Tulsa", state:"OK", zip:"74133",
    price:319000, beds:4, baths:3, sqft:2021, status:"Active",
  ...
  { address:"1401 S Rockford Avenue", city:"Tulsa", state:"OK", zip:"74120",
    price:449000, beds:2, baths:3, sqft:1994, status:"Pending",
  ...
  { address:"1475 S 271st West Avenue", city:"Sand Springs", state:"OK", zip:"74063",
    price:420000, beds:4, baths:2, sqft:2289, status:"Pending",
  ...
  { address:"8431 S Sandusky Avenue", city:"Tulsa", state:"OK", zip:"74137",
    price:359000, beds:4, baths:2, sqft:2374, status:"Pending",
...
// Map a raw feed status to a clean display label, or null to drop the listing.
function normalizeStatus(raw){
  const s = (raw||"").toLowerCase().trim();
  if (!s || s === "active" || s === "new" || s === "for sale") return "Active";
  if (s === "pending" || s === "pending sale") return "Pending";
  if (s.includes("under contract") || s === "contingent" || s === "active under contract") return "Under Contract";
  if (s.includes("coming soon")) return "Coming Soon";
  // sold / closed / expired / withdrawn / cancelled / off market / leased -> drop
  return null;
}
const RANK = { "Active":0, "Coming Soon":1, "Under Contract":2, "Pending":3 };
...
export async function onRequest() {
  ...
  try {
    ...
    if (up.ok) {
      ...
      while ((m = re.exec(xml)) !== null) {
        ...
        const status = normalizeStatus(decode(tag(det,"Status")));
        if (!status) continue; // drop sold/closed/etc.
  ...
  const out = (live ? listings : FALLBACK).slice();
  // Active first, then Coming Soon, Under Contract, Pending; price desc within group.
  out.sort((a,b)=>{
    const r=(RANK[a.status]??9)-(RANK[b.status]??9);
    return r!==0 ? r : (b.price-a.price);
  });
